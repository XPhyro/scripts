...
[ -z "${HEADER_SCRIPTS_SH_CONFIG_+x}" ] && {
    ...
    [ -d "$std_xdg_config_home" ] || mkdir -- "$std_xdg_config_home" \
        || std_logerr "basedir.sh: system does not obey XDG Base Directory Specification"
    [ -d "$std_global_config_home" ] || mkdir -- "$std_global_config_home" \
        || std_logerr "basedir.sh: cannot set up global config directory"
    ...
    [ -d "$std_config_home" ] || mkdir -- "$std_config_home" \
        || std_logerr "basedir.sh: cannot set up script config directory"
}
